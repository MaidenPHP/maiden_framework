...
//chat
$result=mysql_query("SELECT * FROM ".TB_PREFIX."chat ORDER BY `id` DESC");
 if ($result) {
	$i=0;
	while($riga=mysql_fetch_array($result)) {
		$usrinf= get_user($riga['usrid']);
		$body.= "<a href='?pg=profile&usr=".$riga['usrid']."' target='_blank'>".$usrinf['username']."</a> : ".$riga['msg']."<br>";
	}
} 
